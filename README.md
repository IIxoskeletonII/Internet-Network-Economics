...
The headline result is that most of the network-economics story usually told about
stablecoins does **not** survive the data — but the payments-substitution case does, with
conditions.

| Hypothesis | Prediction | Result | Verdict |
|---|---|---|---|
| **H1** Metcalfe's Law | activity scales super-linearly with users (β ≈ 2) | β = 0.98 (USDC), 1.01 (USDT); cannot reject β = 1, strongly reject β = 2 | **Not supported** — scaling is linear, not Metcalfe |
| **H2** Diffusion & institutional gaps | weak-banking countries adopt faster, more so post-2022 | interaction −0.0005 (SE 0.0007), p = 0.46, sign opposite to predicted | **Not supported** |
| **H3** Market concentration | winner-takes-all, rising HHI | full-window trend −14.2 (p = 0.23); but a sharp structural break (Chow interaction p < 1e-9, R² = 0.77) | **Not a monotonic rise** — a regime change instead |
| **H4** Cost friction | on-chain fees far below legacy rails | ~$405 saved on a $10k transfer (ETH & TRX, p < 0.001); TRX cheaper at every size; ETH *not* cheaper for small $200 transfers until the Dencun upgrade | **Supported, with conditions** |

**H1 — network effects are proportional, not super-linear.** In a log-log regression of
transfer activity on active addresses, both major stablecoins land at β ≈ 1 (USDC 0.982,
SE 0.029; USDT 1.014, SE 0.007), cointegrated with HAC(12) errors. A Wald test cannot reject
β = 1 for USDC (p = 0.54) and rejects β = 2 for both at effectively any level. The
Metcalfe's-Law intuition — value scaling with the square of the user base — simply does not
describe stablecoin networks; they scale roughly one-for-one with active addresses. USDC's
slope also shifted across the FTX break (Chow p = 0.003), while USDT's stayed stable
(p = 0.23).

**H2 — no evidence that weak banking infrastructure accelerated adoption.** The headline
two-way fixed-effects interaction (baseline financial access × post-2022) is −0.0005
(SE 0.0007, p = 0.46) across 123 countries, and its sign is the opposite of what the
hypothesis predicted. The institutional-gap story is not visible in this panel.

**H3 — concentration did not follow a clean winner-takes-all trend.** The full-window HHI
trend is small and insignificant (−14.2, p = 0.23). What the data *do* show is a strong
structural break: a Chow interaction of +119.4 (p < 1e-9, R² = 0.77) separating the pre- and
post-2022 regimes. The market restructured rather than steadily concentrating.

**H4 — the cost case holds where it matters most.** Against a modelled legacy remittance
cost, on-chain rails save roughly **$405 on a $10,000 transfer** (ETH +404.7, TRX +419.5,
both p < 0.001). Tron is cheaper than legacy at every transfer size tested, including small
ones (+$8.26 on a $200 transfer). Ethereum is the exception: on a $200 transfer it was
*not* cheaper than legacy over the full window (−$6.53, insignificant), moving to rough
parity only after the April 2024 Dencun upgrade. So the "orders of magnitude cheaper" claim
is true for large-value corridors and for low-fee chains — not universally.

**Answer to the research question.** On this evidence, stablecoins are a genuine *cost*
substitute for correspondent banking in exactly the segment that matters for cross-border
value transfer — larger transfers, and low-fee chains — but the broader network-economics
predictions that would imply runaway displacement do not hold: adoption scales linearly
rather than super-linearly, shows no acceleration in the underbanked economies where
substitution would matter most, and reveals no clean winner-takes-all concentration. The
rails are competitive on price; the network dynamics are more ordinary than the narrative
suggests.

<sub>Full regression tables in `outputs/tables/`; figures in `outputs/figures/`. Every
coefficient above is HAC/cluster-robust as documented per hypothesis below.</sub>
...
```bash
   git clone https://github.com/IIxoskeletonII/Internet-Network-Economics.git
...
**API keys:** only `ETHERSCAN_API_KEY` is required, and only if re-running notebook 01. CoinMetrics Community API, DefiLlama, World Bank WDI/Findex and Tronscan are all public or keyless. The `.env` file is gitignored and must never be committed.
...
Academic project — MSc Data Science and Management, Internet and Network Economics module, LUISS Guido Carli.
